...
function loadXML(){
	...
	if(!xmlData){
		$.ajax({ 			
			type: "GET",
			url: "brokage_betSlip.xml?"+window.dateObj.getTime(),
			dataType: "xml",
			success: onLoadData
		});		
	}
...
function onLoadData(xml){	
	if(startUp){
		teamName=[$(xml).find("team").find("home").text(),$(xml).find("team").find("away").text()];
		$('#homeName').text(teamName[0]);		
		$('#awayName').text(teamName[1]);
		oddsType=$(xml).find("betData").attr('odds');
		defaultOdd=$(xml).find("betData").attr('defaultOdds');	
		defaultCurrency=$(xml).find("betData").attr('defaultCurrency');
	};
	
	score=[$(xml).find("score").find("home").text(),$(xml).find("score").find("away").text()];
	update('score');
	redCard=[$(xml).find("redCard").find("home").text(),$(xml).find("redCard").find("away").text()];	
	update('redCard');
	
	usersXML= $(xml).find("user");
	var i=0;
	users=[];
	$(usersXML).each(function(){	
		idVar = $(this).attr('id');
		name=$(this).text();
		users[i]={value:idVar,id:idVar,name:name};		
		i++;
	});
	
	slipsXML= $(xml).find("slip");
	var j=0;
	betData=[$(xml).find("betData").attr('handicap'),$(xml).find("betData").attr('odds'),[]];
	$(slipsXML).each(function(){	
		goalLline=$(this).attr('goalLine');
		status=$(this).attr('status');
		bestOdds=$(this).attr('bestOdds');
		maxBet=$(this).attr('maxBet');
		totMaxBet=$(this).attr('totMaxBet');	
		minBet=$(this).attr('minBet');
		betData[2][j]=[goalLline,status,bestOdds,maxBet,totMaxBet,minBet,[]];	
		bookieXML = $(this).find("bookie");
		var k=0;
		$(bookieXML).each(function(){
			bookieCode=$(this).attr('id');
			bookieName=$(this).text();
			bookieMaxBet=$(this).attr('maxBet');
			bookiePrice=$(this).attr('price');			
			betData[2][j][6][k]=[bookieCode,bookieName,bookiePrice,bookieMaxBet];			
			k++;		
		});		
		j++;
	});	
...
function genBetSlip(){	
	if(startUp){
		...
		for(a=0; a<betData[2].length; a++){
			...
			$('#slipScroll #SLP_'+a+' .theSlip').append('<div class="placeBetArea"></div>');
			$('#slipScroll #SLP_'+a+' .placeBetArea').append('<table><tr id="row1">'+
				...
				'</tr></table>'+
				'<table id="client"><tr>'+
				'<td colspan="2" id="col1">client id: <input name="cid" type="text" id="cid_'+a+'" class="textField cid"/></td>'+
				'<td colspan="2" id="col2">&nbsp;<input name="cname" type="text" id="cname_'+a+'" class="textField cname" disabled /></td>'+
				'</tr></table>'+
			...
			$('#cid_'+a).data('a',a).autocomplete({
				...
				select:function( event, ui ){
					$(this).val( ui.item.id );
					$('#cname_'+$(this).data('a')).val('('+ui.item.name+')'); 
					return false;
				}
			}).blur(function(){
				if($(this).val()==''){
					$('#cname_'+$(this).data('a')).val('');
				}else if($('#cname_'+$(this).data('a')).val()==''){
					$(this).val('');
				}		
			}).data("ui-autocomplete")._renderItem=function(ul,item){
				return $("<li>").append("<div><a><table><tr><td class='col1'>"+item.id+"</td><td class='col2'>"+item.name+"</td></tr></table></a></div>").appendTo(ul);
			};
			...
			for(b=0; b<betData[2][a][6].length; b++){
				$('#slipScroll #SLP_'+a+' .bookiesFrame table').append('<tr id="row'+b+'">'+
			...
			$('#slipScroll #SLP_'+a+' .collapseBar').append('<div class="upper">'+
				'<div id="openBTN"></div>'+
		...
		for(a=0; a<betData[2].length; a++){
			$('#slipScroll #SLP_'+a+' .placeBetArea #row2 #col2').html(betData[2][a][2]);
			$('#slipScroll #SLP_'+a+' .placeBetArea #row2 #col4').html(betData[2][a][4]);
			$('#slipScroll #SLP_'+a+' .placeBetArea #row3 #col2').html(betData[2][a][3]);
			$('#slipScroll #SLP_'+a+' .placeBetArea #row3 #col4').html(betData[2][a][5]);			
			for(b=0; b<betData[2][a][6].length; b++){
				$('#slipScroll #SLP_'+a+' .bookiesFrame table #row'+b+' .col3').html(betData[2][a][6][b][2]);
				$('#slipScroll #SLP_'+a+' .bookiesFrame table #row'+b+' .col4').html(betData[2][a][6][b][3]);						
			}	
		}
	}
	setTimeout(function(){loadXML()},reloadTime);
};
...
function popupWidth(mode,num){	
	...
	if(!isSafari()){
		...
		if(mode=='startUp'){
			...
			setTimeout(function(){adjustHeight()},animTime+10);
		}else{
			$('#slipArea').css('margin','0px');
			$('#slipScroll').animate({width:slipW}, animTime+10);	
			setTimeout(function(){adjustHeight()},animTime+10);	
		};
	}else{		
		if(mode=='startUp'){
...
function adjustHeight(){
	...
	$('.collapseBar .lower').height($('.collapseBar').height()-$('.collapseBar .upper').height());
};

...
function genBetSlip(){	
	...
	for(a=0; a<betData[2].length; a++){
		...
		$('#slipScroll #SLP_'+a+' .theSlip').append('<div class="selectAllBox selectAllBoxRookie"><div id="col1">price</div><div id="col2">max. bet</div></div>');
		
		$('#slipScroll #SLP_'+a+' .theSlip').append('<div class="frameWindowRookie"><div class="bookiesFrame bookiesFrameRookie"><table width="0" border="0" cellspacing="0" cellpadding="3"></table></div></div>');
		$('#slipScroll #SLP_'+a+' .theSlip #selectAll_'+a).data('childNum',betData[2][a][6].length);		
		
		
		for(b=0; b<betData[2][a][6].length; b++){
			$('#slipScroll #SLP_'+a+' .bookiesFrame table').append('<tr id="bookie'+b+'">'+
				'<td class="col1">'+betData[2][a][6][b][0]+'</td>'+
				'<td class="col2">'+betData[2][a][6][b][1]+betData[2][a][6][b][2]+'</td>'+
				'</tr>');	
			//check box action
			$('#slipScroll #SLP_'+a+' .bookiesFrameRookie  #bookie'+b).data('slipNum',a).data('price',betData[2][a][6][b][0]).data('maxBet',betData[2][a][6][b][2]).click(function(){
				$('#oddTXT_'+$(this).data('slipNum')).val($(this).data('price'));
				$('#stakeTXT_'+$(this).data('slipNum')).val($(this).data('maxBet'));
			});		
		}	
		$('#slipScroll #SLP_'+a+' .collapseBar').append('<div class="upper upperRookie">'+
			'<div id="openBTN"></div>'+
...
function adjustHeight(){
	...
	$('.collapseBar .lower').height($('.collapseBar').height()-$('.collapseBar .upperRookie').height());
};
